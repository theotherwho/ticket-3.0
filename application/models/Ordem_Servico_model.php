...
class Ordem_Servico_model extends CI_Model {
    ...
    public function salvar() {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->session->set_flashdata('danger', 'Não foi possível cadastrar Ordem de Serviço.');
        } else {
            $this->db->trans_commit();
            $this->session->set_flashdata('success', 'Ordem de Serviço cadastrada com sucesso.');
        }
    }
    ...
    public function atualizar($id) {
        ...
        $data = ["id" => $id, "status" => $this->input->post('status'), "andamento" => $this->input->post('andamento')];
        $sql = 'INSERT INTO STATUS_OS (ID_STATUS_OS, ID_ORDEM_SERVICO, ID_STATUS, ID_USUARIO, DESCRICAO, DATA_ANDAMENTO) VALUES ((SELECT NVL(MAX(ID_STATUS_OS) + 1, 1) FROM STATUS_OS), ?, ?, 1, ?, SYSDATE)';
        $this->db->query($sql, $data);
        ...
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            $this->session->set_flashdata('danger', 'Não foi possível atualizar Ordem de Serviço.');
        } else {
            $this->db->trans_commit();
